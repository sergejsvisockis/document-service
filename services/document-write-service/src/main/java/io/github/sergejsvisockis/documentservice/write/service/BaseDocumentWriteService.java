...
public abstract class BaseDocumentWriteService<T> {
    ...
    public SavedDocumentMetadata process(T request) {
        T validatedRequest = validate(request);

        Resource pdfDocument = generate(validatedRequest);

        SavedDocumentMetadata savedDocumentMetadata = sendToStorage(pdfDocument);

        return writeMetadata(savedDocumentMetadata);
    }
     ...
     */
    abstract Resource generate(T request);
     ...
     * @return {@link SavedDocumentMetadata} saved document metadata
     */
    abstract SavedDocumentMetadata sendToStorage(Resource request);
     ...
     * @return {@link SavedDocumentMetadata} saved document metadata.
     */
    abstract SavedDocumentMetadata writeMetadata(SavedDocumentMetadata request);
